...
# 这个文件是
    ...
    @overrides    # 看看这个里面的read如何读取数据,看看到底数据怎么tigong. 这个地方需要写_, 不写_错了, 里面有read和_read算2个函数!!!!!!!!!
    def _read(self, file_path): # 这里面生成了编码!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!------------------------------
...
# 找到X,y
                ...
                try:
                    tokens = [Token(token) for token, tag in tokens_and_tags] # 字符包装一层token类

    ...
    for diff in diffs:
        ...
        if tag == 'equal': # ------------这3中操作: keep,删除,插入这些都是直接建立变换即可. 后续的replace需要复杂讨论.目标是尽可能用g-变换来降低打tag的数量.从而增强模型的鲁棒性.
            ...
            if edits:
                ...
                continue
# 进行对齐.
            # normalize alignments if need (make them singleton)        # 处理其他变化------也就是g变换.和replace变换.
...
# 2020-11-20,9点11 现在发现的问题是.
    if 1:# 这里面是加空格版本, 这种处理方式会让全部的最大512token变成256了.下面研究一下如何避免这个bug.
        source_data=[correct(i) for i in source_data ]
        target_data=[correct(i) for i in target_data ]
        print("补完空格了")# 这里面处理必须加空格,只能后面编码时候再补回来了.

...
#----------------------chakan zhehang!!!!!!!!!!!!
    ...
    iterator = BucketIterator(batch_size=args.batch_size,           # 在这里面进行的bert编码计算!!!!!!!!!!!!!!!!!!!!!!!
                              sorting_keys=[("tokens", "num_tokens")],
                              biggest_batch_first=True,
                              max_instances_in_memory=args.batch_size * 20000,
                              instances_per_epoch=instances_per_epoch,
                              )
    iterator.index_with(vocab) #是用这个词典进行编码.------------------------------############读取数据的真正方法:      iterator(train_data).__next__()   把数据扔给iterator,然后返回一个生成器,调用netx方法进行组个读取.1
...
if __name__ == '__main__':
    ...
    args.source='wi_loc_src.txt'  # 这个里面输入的是错误的中文句子
    args.target='wi_loc_tgt.txt' # 这个里面输入的是正确的中文句子.

...
def get_token_indexers(model_name, max_pieces_per_token=5, lowercase_tokens=True, special_tokens_fix=0, is_test=False):
    bert_token_indexer = PretrainedBertIndexer( # 调用hf里面tokenizer
...
def main(args):
    ...
    # read datasets
    weights_name = 'hfl/chinese-xlnet-base'
...
#   list(train_data)
    ...
    if 1: # 生成字典. 利用数据集生成字典!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
        ...
        #-------------需要对这个from_instances进行修改,看里面如何生成我的更大字典.这个是已经封装好的了.所以不用看了.改上面数据才行.---------强制根据数据集改成我们用的标签.
        new_vocab = Vocabulary.from_instances(train_data,
                                          max_vocab_size={'tokens': args.maxVocab,
                                                          'labels': args.target_vocab_size,
                                                          'd_tags': 2},
                                          tokens_to_add=tokens_to_add) # 建立label 表.
...
#----------------------chakan zhehang!!!!!!!!!!!!
    ...
    new_vocab.save_to_files(os.path.join(args.model_dir, 'vocabulary'))
    from pathlib import Path
    vocabdir=Path(__file__).resolve().parent.parent / os.path.join(args.model_dir, 'vocabulary','labels.txt')
    print("Data is loaded")
    weights_name='hfl/chinese-xlnet-base'  # 这里面替换成了中文的xlnet 进行后续finetune!!!!!!!!!!
    model = get_model(weights_name, new_vocab,
    ...
    iterator.index_with(vocab) #是用这个词典进行编码.
    trainer = Trainer(model=model,
                      optimizer=optimizer,
                      scheduler=scheduler,
                      iterator=iterator,
                      train_dataset=train_data,  # 这里面是训练数据!!!!!!!!
...
if __name__ == '__main__':
    ...
    # args.source='正确预料中文'  # 这个里面输入的是错误的中文句子
    # args.target='错误预料中文' # 这个里面输入的是正确的中文句子.
    ...
    args.cold_steps_count = 4  # 使用这个5002特征的.
    args.n_epoch = 25  # 使用这个5002特征的.
    args.accumulation_size = 1  # 使用这个5002特征的.
    # 其实这个特征还可以进一步优化, 可以设置成,输入平行预料,然后计算特征是什么,然后如果超出5002特征的,就设置新的特征是5003,....
    # 并且前5002个特征的参数还是保持不动.--------------------------------------------------------
    # 目前发现对于单词拼写方面的错误类型还是不够.
    ...
    args.lr = 1e-5
    args.batch_size = 4
    args.target_vocab_size = 99999  # 表示这是个多少的分类任务.无限开大即可,保证所有任务都能成功.
    args.maxVocab=99999# 这个数值目前没用. 因为只对label和d_tag做分类任务.还是写这里,万一以后能用上.



    # args.n_epoch=25 # 必须大于cold,否则存储的参数数量不够.
    # args.oldmodel='/xlnet_0_gector.th'  # 这个是英文的.
    args.oldmodel='/xlnet_zh_gector.th'
    main(args)
    '''

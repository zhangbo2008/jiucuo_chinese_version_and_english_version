...
class Trainer(TrainerBase):
    ...
    def _train_epoch(self, epoch: int) -> Dict[str, float]:
        ...
        if 0:  # 这个模式写0表示xlnet不进行修改, 写1表示xlnet进行修改. 目前结果是写0效果更好.
        ...
        raw_train_generator = self.iterator(self.train_data, num_epochs=1, shuffle=self.shuffle) # 1轮跑完全部.
        train_generator = lazy_groups_of(raw_train_generator, num_gpus)
        '''
        lazy_groups_of 作用就是

        DataLoader(dataset=dealDataset,
                              batch_size=batch_size,
                              shuffle=True,
                              pin_memory=True,
                              collate_fn=mycollate_fn)
                              这个里面的mycollate_fn 作用!!!!!!!!!!
                              拼接sample为一个大树组.   里面的每一个元素都是一个长度为batch_size的大矩阵. 数量为 len(train.data)/batch_size.
        '''
        num_training_batches = math.ceil(self.iterator.get_num_batches(self.train_data) / num_gpus) # 所有batch的数量.
        ...
        for batch_group in train_generator_tqdm: # 逐个迭代. 总共是样本的全部数据.
            batches_this_epoch += 1
            self._batch_num_total += 1
            batch_num_total = self._batch_num_total
# iter_len 表示当前轮里面有多少个样本.
...
# 经过测试这个冷启动非常重要.
        if self.cold_step_count > 0: # 冷启动几个step, 这些step不用更新权重.对于embed网路都freeeze上.只更新后面自己简历的分类器网络.
            ...
            self.model.text_field_embedder._token_embedders['bert'].set_weights(freeze=True) #----------------------------------------------------------这一步非常重要. 一定要冷启动的时候,只改变最后一层的参数. 否则 这么大学习率直接xlnet 崩溃了!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
#  list(self.model.parameters())        list( self.model.text_field_embedder._token_embedders['bert'].parameters())
